/**
 * @file
 * Integrates Premium Calendar data with FullCalendar.
 */
(function (Drupal, drupalSettings, once) {
  Drupal.behaviors.premium_calendar = {
    attach(context, settings) {
      let slotDate = '';
      async function pushUpdate(data) {
        const url = `${drupalSettings.path.baseUrl}premium_calendar/ajax/update/drop/${data.entity_type}/${data.id}?_format=json`;
        let status = 400;
      ...
      function eventProcess(info) {
        ...
        const viewSettings = drupalSettings.premium_calendar[viewIndex];
      ...
      function buildCalendars() {
        once('premiumCalendarBuild', '.premium_calendar--wrapper', context).forEach(
          (calendarEl) => {
            const domId = calendarEl.id;
            if (drupalSettings.premium_calendar.hasOwnProperty(domId)) {
              const settings = drupalSettings.premium_calendar[domId];
